...
4. **The precipitation scenarios DO ship**, and so does what generates them. `stage1-python/precips_c0_{DML,EP}.csv` are
   `stage2-r/markov_chain.Rmd` produces them - 100,000 Monte Carlo draws each. They are *inputs* to the
   stochastic notebooks rather than their outputs - both canonical notebooks read them and neither
   runs without them. A documented exception to invariant 5.
5. **The intermediate CSVs are gitignored.** They are 68 MB of generated scenario output, and
...
6. **No seeds.** The Monte Carlo loops (`range(1000)`, `range(4000)`) do not record a seed, so a
...
7. **No environment lock.** Requires `gurobipy` and a Gurobi licence; the R stage requires
